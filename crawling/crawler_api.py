...
# 내부 모듈 호출
from lib.utils import get_formatted_parts, split_stats_for_json

class SmartCrawler:
	def __init__(self, urls: List[str], keywords: List[str], exclude_keywords: List[str], output_excel: str = "data/crawling_results.xlsx", output_json: str = "data/crawling_summary.json"):
		self.urls = urls
		...
		excel_dir = os.path.dirname(self.output_excel)
		json_dir = os.path.dirname(self.output_json)
		if excel_dir: os.makedirs(excel_dir, exist_ok=True)
			...
			try:
				...
				for row in self.ws_data.iter_rows(min_row=2, values_only=True):
					...
					try:
						...
						pass
				print(f"[DB] 기존 DB 로드 완료. (중복 스킵용 데이터 {len(self.existing_texts)}개 확보)")
			except Exception as e:
				print(f"[WARN] 기존 DB 수집 파일을 읽는 중 오류가 발생했습니다: {e}")
				file_exists = False
	...
	def setup_driver(self) -> bool:
		...
		try:
			print("[SYSTEM] 크롬 브라우저 스텔스 모드를 준비 중입니다...")
	...
	def run_crawler(self) -> bool:
		...
		try:
			for url in self.urls:
				print(f"\n[NETWORK] [{url}] 브라우저 접속 중...")
				try:
					self.driver.get(url)
					print("  [WAIT] 안정성을 위해 화면이 완전히 뜰 때까지 15초간 대기합니다...")
					time.sleep(15)
				except Exception as e:
					print(f"  [ERROR] 접속 에러 (다음 사이트로 건너뜁니다): {e}")
					continue

				for keyword in self.keywords:
					print(f"\n  [SEARCH] [현재 검색어: {keyword}] -> 수집 및 정밀 분석 시작")
					...
					if not targets_to_click:
						print(f"    [INFO] '{keyword}' 관련 새로운 글이 없거나 이미 DB에 수집된 상태입니다.")
						continue

					print(f"    [TARGET] 새로운 글 {len(targets_to_click)}개 발견! 상세 주소 추출을 시작합니다.")
						...
						try:
							...
							if elem_to_click:
								...
								if len(after_windows) > len(before_windows):
									...
									print(f"      [WAIT] [현재 창 이동] 로딩 대기 (15초)...")
									time.sleep(15)
									specific_url = self.driver.current_url
	...
	def _process_success_page(self, specific_url: str, target: Dict[str, Any], current_time: str, origin_url: str, kw_comb: str):
		...
		print(f"      [SUCCESS] 상세 주소 획득 완료! (문서파일: {has_file})")
	...
	def save_all_data(self) -> None:
		...
		try:
			...
			print(f"\n[ERROR] [저장 실패] DB(엑셀) 파일 접근 권한이 잠겨있습니다. 창을 닫아주세요.")
		...
		try:
			...
			print(f"[SAVE] JSON 요약 파일({self.output_json}) DB 저장이 완료되었습니다.")
		except Exception:
			pass

		print("\n" + "="*50)
		print("[SUMMARY] 크롤링 결과 세부 요약")
		print(f"[TIME] 1. 최근 24시간 수집 전체 카운팅: {self.recent_24h_count}건")
		...
		print(f"\n[NEW]  1. 이번 실행 신규 발견 전체 카운팅: {self.new_found_today_count}건")
		print(f"          [SUCCESS] 수집 성공: {self.new_success_count}건")
		print(f"          [FAIL]    수집 실패: {self.new_failure_count}건 (DB에서 순수 주소 확인 가능)")
		print("="*50)